...
use Daycry\ReCaptcha\Config\ReCaptcha3;
...
if (!function_exists('reCaptcha2'))
{
    function reCaptcha2(string $name, array $attributes = [], array $options = [], bool $init = true)
    {
        ...
        if ($init)
        {
            $return .= view_cell('Daycry\ReCaptcha\Cells\ReCaptcha::init_v2', [
                'key' => $config->key,
                'id' => $attributes['id'],
                'options' => $options
            ]);
...
if (!function_exists('reCaptcha3'))
{
    function reCaptcha3(string $name, array $attributes = [], array $options = [], bool $init = true)
    {
        ...
        if ($init)
        {
            $return .= view_cell('Daycry\ReCaptcha\Cells\ReCaptcha::init_v3', [
                'key' => $config->key,
                'id' => $attributes['id'],
                'options' => $options
            ]);
